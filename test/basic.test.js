...
describe('using only basic fields', function() {
  var docs = [
    {
      ...
      stats_friends: 2163,
      apple_push_token:
        'd4b4e7f3361cec05fae848575d7e6e1da2f0dccdf8ccc86a8ff2124d8b0542f6',
  ...
  it('should detect the correct type for each field', function() {
    ...
    assert.equal(
      _.find(users.fields, ['name', 'apple_push_token']).type,
      'String'
    );
    ...
    assert.equal(
      _.find(users.fields, ['name', 'last_address_latitude']).type,
      'Null'
    );
    assert.equal(
      _.find(users.fields, ['name', 'last_address_longitude']).type,
      'Null'
    );
    assert.equal(_.find(users.fields, ['name', 'name']).type, 'String');
    assert.equal(
      _.find(users.fields, ['name', 'stats_friends']).type,
      'Number'
    );
    assert.equal(
      _.find(users.fields, ['name', 'twitter_username']).type,
      'String'
    );
  });
});
