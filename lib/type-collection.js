...
module.exports = Collection.extend(lodashMixin, parentMixin, {
  ...
  addToType: function(value) {
    ...
    type.parse(value);
    if (newTypeAdded && this.parent) {
      this.parent.trigger('change:types.length');
    }

    // trigger probability change event in all direct type children
    this.each(function(type) {
      type.trigger('change:probability');
    });
  },
  serialize: function() {
    this.sort();
