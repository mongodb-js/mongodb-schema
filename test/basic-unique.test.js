...
describe('has_duplicates', function() {
  ...
  it('should have 10000 number values for the `num` field', function() {
    assert.equal(
      _.find(_.find(schema.fields, ['name', 'num']).types, ['name', 'Number'])
        .values.length,
      10000
    );
  });
  ...
  it('should have 100 string values for the `str` field', function() {
    assert.equal(
      _.find(_.find(schema.fields, ['name', 'str']).types, ['name', 'String'])
        .values.length,
      100
    );
  });
...
describe('unique', function() {
  ...
  it('should have unique of 3 for `_id`', function() {
    assert.equal(
      _.find(_.find(schema.fields, ['name', '_id']).types, ['name', 'Number'])
        .unique,
      3
    );
  });
  ...
  it('should have unique of 1 for `registered` type Boolean', function() {
    assert.equal(
      _.find(
        _.find(schema.fields, ['name', 'registered']).types,
        'name',
        'Boolean'
      ).unique,
      1
  ...
  it('should have unique of 1 for `code`', function() {
    assert.equal(
      _.find(_.find(schema.fields, ['name', 'code']).types, ['name', 'Null'])
        .unique,
      1
    );
  });
  ...
  it('should have unique of 2 for `int32`', function() {
    console.log(
      'should have unique of 2 for `int32`',
      _.find(schema.fields, ['name', 'int32']).types
    );
    console.log(
      _.find(_.find(schema.fields, ['name', 'int32']).types, ['name', 'Int32'])
        .values
    );
    assert.equal(
      _.find(_.find(schema.fields, ['name', 'int32']).types, ['name', 'Int32'])
  ...
  it('should have unique of 2 for `date`', function() {
    assert.equal(
      _.find(_.find(schema.fields, ['name', 'date']).types, ['name', 'Date'])
        .unique,
      2
    );
  });
  ...
  it('should have duplicates for `registered`', function() {
    assert.equal(
      _.find(schema.fields, ['name', 'registered']).has_duplicates,
      true
    );
  });
});
