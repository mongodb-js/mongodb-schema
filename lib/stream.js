...
var addToValue = function(type, value) {
  ...
  // object size computation only supported for values other than null, undefined
  if (!_.isNull(value) && !_.isUndefined(value)) {
    totalValueSize += BSON.calculateObjectSize(value);
  }
 ...
 *
 * @param {Object} options    options for schema analysis configuration.
 *
 * @option {Number} memoryLimit    impose a memory limit in bytes and stop
 *                                 sampling when reached.
 */
...
module.exports = function parse(options) {
  ...
  if (options.memoryLimit) {
    debug('memory limit of', options.memoryLimit, 'enforced.');
  }
  ...
  var parser = es.through(function write(obj) {
    if (options.memoryLimit && totalValueSize > options.memoryLimit) {
      schema.memoryLimitReached = true;
      return;
    }
