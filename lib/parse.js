...
var debug = require('debug')('mongodb-schema:parse');
...
mapType = function(value, prefix) {
  ...
  switch (type.name) {
    case 'Document':
      ...
      break;
    case 'Array':
      var grouped = _.groupBy(_.map(value, function(value) {
        return mapType(value, prefix);
      }), 'name');
      type.types = _.mapValues(grouped, function(values) {
        ...
        _.each(values, function(value) {
          return _.merge(result, value, function(objectValue, sourceValue, key) {
            if (key === 'count') {
              return (_.isUndefined(objectValue) ? 0 : objectValue) + sourceValue;
            }
            if (key === 'values') {
              debug('values merge', objectValue, sourceValue);
              return (_.isUndefined(objectValue) ? [] : objectValue).concat(sourceValue);
            }
          });
        });
        return result;
...
module.exports = function parse(model) {
  /* eslint no-sync: 0 */
  return es.pipeline(mapper, reducer, finalizer, es.map(function(data, cb) {
    if (model) {
      model.set(data, {
        parse: true
      });
