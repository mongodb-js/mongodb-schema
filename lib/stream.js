...
var extractStringValueFromBSON = function(value) {
  console.log('extractStringValueFromBSON', value);
  ...
  if (tag === 'types') {
    _.each(schema, function(type) {
      ...
      if (type.name === 'Null' || type.name === 'Undefined') {
        ...
        type.unique = _.uniqBy(
          type.values,
          extractStringValueFromBSON
        ).length;
...
module.exports = function parse(options) {
  ...
  var addToType = function(path, value, schema) {
    ...
    var typeName = options.semanticTypes
      ? getSemanticType(value, path) || bsonType
      : bsonType;
    ...
    if (typeName === 'Array') {
      ...
      type.values = _.get(
        type,
        'values',
        bsonType === 'String' ? new Reservoir(100) : new Reservoir(10000)
      );
